...
# Import translations from the main translations file
...
# Language dictionaries are now imported from translations.py

# Use the imported translations from translations.py
# The translations dictionary is already imported above
...
class FileManagerApp:
    def __init__(self, root):
        ...
        self.temp_files = []    # Oluşturulan geçici dosyaların yolları (kapatıldığında silinecek)
        ...
        try:
            ...
            icon_path = os.path.join(os.path.dirname(__file__), "listekolay.png")
...
# Final: Genişletilmiş ve mantıksal olarak gruplanmış 6 dosya kategorisi
        ...
        # Config yüklendikten sonra tooltip'leri doğru dilde güncelle
        self.update_all_tooltips()
        
        # Ayarlar bölümündeki tooltip'leri özellikle güncelle
        self.update_settings_tooltips()
        
        # Tema tooltip'lerini özellikle güncelle (dil yüklendikten sonra)
    ...
    def get_text(self, key):
        ...
        # Use main translations system - current language first, then English, then key itself
    ...
    def create_gui(self):
        ...
        # Load and display program icon in title area
        try:
            base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
            icon_path = os.path.join(base_path, "listekolay.png")
            if os.path.exists(icon_path):
                # Load and resize icon for UI title area
                pil_icon = Image.open(icon_path)
                # Resize to appropriate size for title area (48x48 pixels)
                pil_icon = pil_icon.resize((60, 60), get_pil_resize_method())
                self.title_icon = ImageTk.PhotoImage(pil_icon)
                ...
                icon_label.pack(side=tk.LEFT, padx=(0, 8), pady=(0, 2))
            else:
                logging.warning(f"UI icon file not found: {icon_path}")
        except Exception as e:
            logging.error(f"Error loading UI icon: {str(e)}")
        ...
        # Store references for theme radio buttons for later tooltip updates
        self.light_radio = light_radio
        self.dark_radio = dark_radio
        ...
        self.create_tooltip(self.toggle_left_panel_btn, self.get_text("toggle_panel"))
        ...
        # Store references for format checkboxes for later tooltip updates
        self.text_cb = text_cb
        self.excel_cb = excel_cb
        self.word_cb = word_cb
        self.html_cb = html_cb
        ...
        # Arama ipucu ekle - dinamik çeviri ile
        self.create_tooltip(self.file_search_entry, self.get_text("search_filter_tooltip"))
        ...
        self.create_tooltip(info_btn, self.get_text("tooltip_info_icon"))
        ...
        # Store website link reference for tooltip updates
    ...
    def show_context_menu(self, event):
        ...
        if item:
            ...
            # Ensure context menu exists before showing it
            if not hasattr(self, 'context_menu') or self.context_menu is None:
    ...
    def on_search_focus_in(self, event):
        ...
        current_text = self.file_search_entry.get()
        # Check if current text is the placeholder text for any language
        current_placeholder = self.get_text("search_files")
    ...
    def delete_selected_files(self):
        ...
        for item in selected_items:
            ...
            if values:
                file_name = values[0]
                file_path = os.path.join(values[2], file_name)
                file_paths.append(file_path)
        
        if not file_paths:
            return
        
        # Confirm deletion using translation keys
        result = messagebox.askyesno(
            self.get_text("confirm_delete_file"),
            self.get_text("confirm_delete_file_message")
        )
        ...
        if result:
            ...
            for file_path in file_paths:
                try:
                    ...
                    messagebox.showerror(
                        self.get_text("error"),
                        f"{self.get_text('delete_error')}: {str(e)}"
                    )
            
            if deleted_count > 0:
                messagebox.showinfo(
                    self.get_text("operation_completed"),
                    self.get_text("files_deleted_successfully")
                )
                # Refresh file list
                self.load_files()
    ...
    def copy_selected_files(self):
        ...
        if file_paths:
            ...
            self.update_status(f"{len(file_paths)} {self.get_text('files_copied_to_clipboard')}")
    ...
    def move_selected_files(self):
        ...
        dest_folder = filedialog.askdirectory(title=self.get_text("select_destination_folder"))
        if not dest_folder:
            return
        ...
        for file_path, file_name in file_paths:
            try:
                ...
                messagebox.showerror(
                    self.get_text("error"),
                    f"{self.get_text('move_error')}: {str(e)}"
                )
        
        if moved_count > 0:
            messagebox.showinfo(
                self.get_text("info"),
                f"{moved_count} {self.get_text('files_moved_successfully')}"
            )
            # Refresh file list
            self.load_files()
    ...
    def cut_selected_files(self):
        ...
        if file_paths:
            # Store cut files for later paste operation
            self.cut_files = file_paths
            self.update_status(f"{len(file_paths)} {self.get_text('files_cut_successfully')}")
    ...
    def rename_selected_file(self):
        ...
        old_name = values[0]
        old_path = os.path.join(values[2], old_name)
        
        if not os.path.isfile(old_path):
            return
        
        # Get new name from user using translation keys
        new_name = simpledialog.askstring(
            self.get_text("rename_file_dialog"),
            f"{self.get_text('rename_file_message')} '{old_name}':",
        ...
        if new_name and new_name != old_name:
            ...
            if not new_name.strip():
                messagebox.showerror(
                    self.get_text("error"),
                    self.get_text("invalid_filename")
                )
                return
            
            try:
                new_path = os.path.join(values[2], new_name)
                ...
                messagebox.showinfo(
                    self.get_text("operation_completed"),
                    self.get_text("file_renamed_successfully")
                )
                # Refresh file list
                self.load_files()
            except Exception as e:
                messagebox.showerror(
                    self.get_text("operation_failed"),
    ...
    # Preview context menu methods
    def open_preview_file(self):
        """Open preview file with default application"""
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            self.open_file(self.selected_preview_path)

    def open_preview_file_location(self):
        """Open preview file location"""
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            self.open_file_location_by_path(self.selected_preview_path)

    def preview_selected_preview_file(self):
        """Preview the selected preview file"""
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            self.create_file_preview_window(self.selected_preview_path)

    def copy_preview_filename_to_clipboard(self):
        """Copy preview filename to clipboard"""
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            filename = os.path.basename(self.selected_preview_path)
            self.root.clipboard_clear()
    ...
    def copy_preview_filepath_to_clipboard(self):
        """Copy preview file path to clipboard"""
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            self.root.clipboard_clear()
            self.root.clipboard_append(self.selected_preview_path)
            self.update_status(self.get_text("copied_to_clipboard"))
    ...
    def delete_preview_file(self):
        ...
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            ...
            if result:
                try:
                    os.remove(self.selected_preview_path)
                    messagebox.showinfo(
                        self.get_text("operation_completed"),
                        self.get_text("files_deleted_successfully")
                    )
        ...
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            ...
            if dest_folder:
                try:
                    ...
                    shutil.move(self.selected_preview_path, dest_path)
                    messagebox.showinfo(
                        self.get_text("info"),
                        f"1 {self.get_text('files_moved_successfully')}"
                    )
                    # Refresh preview
                    self._update_preview_panel()
                except Exception as e:
    ...
    def rename_preview_file(self):
        """Rename preview file"""
        if hasattr(self, 'selected_preview_path') and self.selected_preview_path:
            old_name = os.path.basename(self.selected_preview_path)
            new_name = simpledialog.askstring(
                self.get_text("rename_file_dialog"),
                f"{self.get_text('rename_file_message')} '{old_name}':",
                initialvalue=old_name
            ...
            if new_name and new_name != old_name:
                ...
                try:
                    new_path = os.path.join(os.path.dirname(self.selected_preview_path), new_name)
                    ...
                    os.rename(self.selected_preview_path, new_path)
                    messagebox.showinfo(
                        self.get_text("operation_completed"),
                        self.get_text("file_renamed_successfully")
                    )
    ...
    def show_preview_context_menu(self, event, file_path):
        """Show context menu for preview items"""
        self.selected_preview_path = file_path
        try:
    ...
    def open_file_location_by_path(self, file_path):
        """Open file location by file path"""
        try:
    ...
    def change_language(self, event=None):
        ...
        if new_lang in self.languages:
            ...
            # Update all tooltips with new language
            self.update_all_tooltips()

            # Update format checkboxes tooltips
            self.update_format_tooltips()
            ...
            if current_status in ["Hazır", "Ready", "جاهز", "Bereit", "Prêt", "Готово", "Listo", "Pronto", "آماده", "تیار", "तैयार", "准备就绪", "準備完了", "Pronto"]:
                self.update_status(self.get_text("ready"))

            # Save config after language change
            self.save_config()
    ...
    def update_search_placeholder(self):
        """Update search placeholder text with current language"""
        try:
            if hasattr(self, 'file_search_entry'):
                current_text = self.file_search_entry.get()
                placeholder_text = self.get_text("search_files")

                # Sadece kutu boşsa veya şu anki dile göre placeholder varsa güncelle
                if not current_text or current_text == self.last_placeholder_text:
                    self.file_search_entry.delete(0, tk.END)
                    self.file_search_entry.insert(0, placeholder_text)
                    self.file_search_entry.config(fg='gray')
                    self.last_placeholder_text = placeholder_text
        except Exception as e:
            logging.error(f"Error updating search placeholder: {str(e)}")
    ...
    def update_context_menu(self):
        ...
        try:
            ...
            # Add menu items with current language
            ...
            # Update preview context menu if it exists
            ...
            logging.error(f"Error updating context menu: {str(e)}")

    def update_settings_tooltips(self):
        """Update settings section tooltips specifically"""
        try:
            # Ayarlar bölümündeki tooltip'leri özellikle güncelle
            ...
            if hasattr(self, 'sort_dropdown'):
                self.create_tooltip(self.sort_dropdown, self.get_text("tooltip_sort_criteria"))
            if hasattr(self, 'text_cb'):
                self.create_tooltip(self.text_cb, self.get_text("text_format_info"))
            if hasattr(self, 'excel_cb'):
                self.create_tooltip(self.excel_cb, self.get_text("excel_format_info"))
            if hasattr(self, 'word_cb'):
                self.create_tooltip(self.word_cb, self.get_text("word_format_info"))
            if hasattr(self, 'html_cb'):
                self.create_tooltip(self.html_cb, self.get_text("html_format_info"))
        except Exception as e:
            logging.error(f"Error updating settings tooltips: {str(e)}")

    def update_all_tooltips(self):
        """Update all tooltips with current language"""
        try:
            # Update main button tooltips
            ...
            if hasattr(self, 'toggle_left_panel_btn'):
                self.create_tooltip(self.toggle_left_panel_btn, self.get_text("toggle_panel"))
            if hasattr(self, 'show_filter_btn'):
                self.create_tooltip(self.show_filter_btn, self.get_text("tooltip_file_category"))
            ...
            for widget in self.main_frame.winfo_children():
                self._find_and_update_info_icon_tooltip(widget)
                
            # Ayarlar bölümü tooltip'lerini de güncelle
            self.update_settings_tooltips()
                
        except Exception as e:
            logging.error(f"Error updating tooltips: {str(e)}")

    def _find_and_update_info_icon_tooltip(self, widget):
        """Find info icon button and update its tooltip recursively"""
        try:
            # Check if this widget is a button with info icon text
            if isinstance(widget, tk.Button):
                button_text = widget.cget("text")
                if button_text == "ℹ️":  # Info emoji
                    self.create_tooltip(widget, self.get_text("tooltip_info_icon"))
                    return
            
            # Recursively check child widgets
            for child in widget.winfo_children():
                self._find_and_update_info_icon_tooltip(child)
        except Exception as e:
            logging.error(f"Error finding info icon: {str(e)}")

    def update_format_tooltips(self):
        """Update format checkbox tooltips"""
        try:
            if hasattr(self, 'text_cb'):
                self.create_tooltip(self.text_cb, self.get_text("text_format_info"))
            if hasattr(self, 'excel_cb'):
            ...
            logging.error(f"Error updating format tooltips: {str(e)}")

    def update_view_mode_tooltips(self):
        """Update view mode button tooltips"""
        try:
            if hasattr(self, 'list_view_btn'):
                self.create_tooltip(self.list_view_btn, self.get_text("tooltip_list_view"))
            ...
            logging.error(f"Error updating view mode tooltips: {str(e)}")

    def update_theme_tooltips(self):
        """Update theme radio button tooltips"""
        try:
            # Tema butonlarının tooltip'lerini güncelle
            if hasattr(self, 'light_radio') and self.light_radio and self.light_radio.winfo_exists():
                # Mevcut tooltip'i tamamen temizle
                try:
                    self.light_radio.unbind("<Enter>")
                    self.light_radio.unbind("<Leave>") 
                    self.light_radio.unbind("<Motion>")
                    # Eğer mevcut tooltip penceresi varsa kapat
                    if hasattr(self.light_radio, 'tooltip'):
                        try:
                            self.light_radio.tooltip.destroy()
                            delattr(self.light_radio, 'tooltip')
                        except:
                            pass
                except:
                    pass
                # Yeni tooltip'i oluştur
                self.create_tooltip(self.light_radio, self.get_text("light_mode"))
                
            if hasattr(self, 'dark_radio') and self.dark_radio and self.dark_radio.winfo_exists():
                # Mevcut tooltip'i tamamen temizle
                try:
                    self.dark_radio.unbind("<Enter>")
                    self.dark_radio.unbind("<Leave>")
                    self.dark_radio.unbind("<Motion>")
                    # Eğer mevcut tooltip penceresi varsa kapat
                    if hasattr(self.dark_radio, 'tooltip'):
                        try:
                            self.dark_radio.tooltip.destroy()
                            delattr(self.dark_radio, 'tooltip')
                        except:
                            pass
                except:
                    pass
                # Yeni tooltip'i oluştur
                self.create_tooltip(self.dark_radio, self.get_text("dark_mode"))
        except Exception as e:
            logging.error(f"Error updating theme tooltips: {str(e)}")
    ...
    def load_config(self):
        ...
        try:
            ...
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                # Pencere boyutu ve konumu ayarlanıyor
                if "window_geometry" in config and isinstance(config["window_geometry"], str) and "x" in config["window_geometry"]:
                    try:
                        self.root.geometry(config["window_geometry"])
                        logging.info(f"Pencere boyutu yüklendi: {config['window_geometry']}")
                    except Exception as e:
                        logging.warning(f"Pencere boyutu ayarlanırken hata oluştu: {str(e)}")
